...
export default function UsagePerDayLlama() {
  const { dark } = useDarkMode();
  const [usage, setUsage] = useState<number>(0)
  const [loading, setLoading] = useState(true);
  const MAX_TOKENS = 14400;
  ...
  const fetchToken = async () => {
    try {
      const res = await fetch("/api/admin/countUsagePerDayLlama", {
        method: "POST",
      });
        ...
        {loading ? (
          ...
          <>
            ...
            <p className="mt-2 font-medium">
              Total penggunaan hari ini: {usage} dari 14.400 token
            </p>
